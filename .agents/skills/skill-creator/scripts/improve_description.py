...
by calling `csgdaa-code run` as a subprocess (same auth pattern as run_eval.py —
uses the session's csgdaa-code auth, no separate API key needed).
"""
...
def _call_agent(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run `csgdaa-code run` with the prompt and return the text response.

    Prompt goes over stdin (not argv) because it embeds the full SKILL.md
    body and can easily exceed comfortable argv length.
    """
    cmd = ["csgdaa-code", "run", prompt, "-o", "text"]
    if model:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"csgdaa-code run exited {result.returncode}\nstderr: {result.stderr}"
        )
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; `csgdaa-code run` is one-shot, so we
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_agent(shorten_prompt, model)
