...
An earlier version of the skill labelled itself "Read-Only" in the skill body but allowed write command suggestions. This created a false sense of security. **Resolved**: the read-only label and restrictions have been removed. The skill now has full kubectl access, which is honest about its actual capability.
...
The granular verb-level scoping is a strong pattern. The `Bash(ls:*)` entry could be replaced with `Glob` if surface area reduction is desired.

## Sources

- `skill/SKILL.md` — allowed-tools definition
